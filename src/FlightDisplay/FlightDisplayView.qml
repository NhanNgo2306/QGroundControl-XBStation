...
QGCView {
    ...
    property bool   _mainIsMap:             (QGroundControl.videoManager.hasVideo || XBStationManager.links.length > 0) ? QGroundControl.loadBoolGlobalSetting(_mainIsMapKey,  true) : true
    property bool   _isPipVisible:          (QGroundControl.videoManager.hasVideo || XBStationManager.links.length > 0) ? QGroundControl.loadBoolGlobalSetting(_PIPVisibleKey, true) : false
    ...
    QGCViewPanel {
        ...
        Item {
            ...
            anchors.left:   _panel.left
            anchors.bottom: _panel.bottom
            visible:        (QGroundControl.videoManager.hasVideo || XBStationManager.links.length > 0) && (!_mainIsMap || _isPipVisible)
            ...
            Loader {
                id:             cameraLoader
                anchors.fill:   parent
                visible:        !QGroundControl.videoManager.isGStreamer && XBStationManager.links.length == 0
                source:         QGroundControl.videoManager.uvcEnabled ? "qrc:/qml/FlightDisplayViewUVC.qml" : "qrc:/qml/FlightDisplayViewDummy.qml"
            }
            //-- XB Video Streaming
            XBFlightDisplayViewVideo {
                id:             xbVideoStreaming
                anchors.fill:   parent
        ...
        QGCPipable {
            ...
            anchors.bottom:     _panel.bottom
            anchors.margins:    ScreenTools.defaultFontPixelHeight
            visible:            (QGroundControl.videoManager.hasVideo || XBStationManager.links.length > 0) && !QGroundControl.videoManager.fullScreen && _flightVideo.state != "popup"
            ...
            onShowXBSettingsDialog: {
                showDialog(xbSettingsDialogComponent, qsTr("XB Settings"), qgcView.showDialogDefaultWidth, StandardButton.Close);
            }
        }
