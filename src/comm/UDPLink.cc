...
{
//    qDebug() << "UDP write byte: "<<data.size() << qgcApp()->toolbox()->linkManager()->links().size();
//    for(int i = 0; i < qgcApp()->toolbox()->linkManager()->links().size(); i++) {
//        qDebug()<< qgcApp()->toolbox()->linkManager()->links()[i]->isConnected() << "-" << qgcApp()->toolbox()->linkManager()->links()[i]->highLatency() << "-" <<qgcApp()->toolbox()->linkManager()->links()[i]->getLinkConfiguration()->name();
//    }
    if (!_socket) {
        return;
    }
