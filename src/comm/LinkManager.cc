...
LinkInterface* LinkManager::createConnectedLink(SharedLinkConfigurationPointer& config, bool isPX4Flow)
{
    ...
    case LinkConfiguration::TypeTcp:
        ...
        break;
    case LinkConfiguration::TypeXB:
        pLink = new XBLink(config);
        break;
#ifdef QGC_ENABLE_BLUETOOTH
                    ...
                    if(settings.contains(root + "/name")) {
                        ...
                        if(!name.isEmpty()) {
                            ...
                            case LinkConfiguration::TypeTcp:
                                ...
                                break;
                            case LinkConfiguration::TypeXB:
                                pLink = dynamic_cast<LinkConfiguration*>(new XBConfiguration(name));
                                break;
#ifdef QGC_ENABLE_BLUETOOTH
                            case LinkConfiguration::TypeBluetooth:
    ...
    if (config) {
        ...
        if (config->name() == "Unnamed") {
            ...
            case LinkConfiguration::TypeTcp: {
                ...
                break;
            case LinkConfiguration::TypeXB: {
                XBConfiguration* xbConfiguration = dynamic_cast<XBConfiguration*>(config);
                config->setName((xbConfiguration->username() != "") ? xbConfiguration->username() : config->name());
//                config->setName(QString("XB Station %1:%2").arg(config->name()).arg(xbConfiguration->username()));
                break;
            }
#ifdef QGC_ENABLE_BLUETOOTH
            case LinkConfiguration::TypeBluetooth: {
