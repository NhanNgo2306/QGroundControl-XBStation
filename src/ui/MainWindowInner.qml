...
Item {
    ...
    property string formatedXBMessage:  _cameraControl.formatedMessage
    ...
    function showXBMessage(message) {
        if(QGroundControl.videoManager.fullScreen) {
            xbMessageQueue.push(message)
        }
    }
    ...
    function formatMessage(message) {
        ...
        message = message.replace(new RegExp("<#S>", "g"), "color: " + qgcPal.successText + "; font: " + (ScreenTools.defaultFontPointSize.toFixed(0) - 1) + "pt monospace;");
        return message;
    }
    ...
    function showXBMessageArea() {
        var currentlyVisible = xbMessageArea.visible
        if(!currentlyVisible) {
            if(XBStationManager.links.length > 0) {
                xbMessageText.text = formatMessage(_cameraControl.formatedMessages)
                //-- Hack to scroll to last message
                for (var i = 0; i < _cameraControl.messageCount; i++)
                    xbMessageFlick.flick(0,-5000)
                _cameraControl.resetMessages()
            } else {
                xbMessageText.text = qsTr("No Messages")
            }
            xbMessageArea.visible = true
        }
    }
...
//        anchors.horizontalCenter:   parent.horizontalCenter
        ...
        QGCFlickable {
            ...
            anchors.fill:       parent
            contentHeight:      xbMessageText.height
            contentWidth:       xbMessageText.width
            pixelAligned:       true
            clip:               true
            TextEdit {
                id:             xbMessageText
    ...
    Connections {
        target: _cameraControl
        onShowXBMessage: {
            if(xbMessageArea.visible) {
                xbMessageText.append(formatMessage(formatedXBMessage))
                //-- Hack to scroll down
                xbMessageFlick.flick(0,-500)
            }
        }
    }
